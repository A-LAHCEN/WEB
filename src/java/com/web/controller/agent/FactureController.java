...
import com.web.entity.WebReglement;
...
public class FactureController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("SUIVIE") != null) {
            ...
            WebFacture facture=manager.find(Long.valueOf(request.getParameter("id")));
            facture.setEtat(true);
            try {
                manager.edit(facture);
    ...
    private boolean searchAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Date date = null;
        if (request.getParameter("client") != null && request.getParameter("date") != null && ! request.getParameter("date").equals("")) {
            try {
    ...
    private boolean createAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.setAttribute("types", new WebType []{WebType.FACTURE,WebType.LIVRAISON});
    ...
    private void suivieAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        if (request.getParameter("DELETE") != null) {
            
        } else if (request.getParameter("CREATE") != null) {
            WebEntityManager<WebFacture> manager = new WebEntityManager<>(WebFacture.class);
            WebFacture facture=manager.find(Long.valueOf(request.getParameter("facture")));
            WebEntityManager<WebReglementMode> rm = new WebEntityManager<>(WebReglementMode.class);
            request.setAttribute("modes", rm.findEntities());
            request.setAttribute("facture", facture);
            request.getRequestDispatcher("/WEB-INF/public/WebAgent/suivie/create.jsp").forward(request, response);
        } else if (request.getParameter("SAVE") != null) {
            WebEntityManager webEntityManager=new WebEntityManager();
            WebReglement reglement=new WebReglement(request);
            if(reglement.isValid()){
                webEntityManager.create(reglement);
               
        if(reglement.getFacture().getPayed()>= reglement.getFacture().getTotal()){
            webEntityManager.execute("WebFacture.updateEtat", true,reglement.getFacture().getId());
        }
            }
            indexAction(request, response);
        }
